...
LIC :: LIC()
{
	...
	line_square_clip_max = 100000.0f;
	vector_component_min = 0.050000F;
	FD.loadFiles();
	FD.convertEndian(); 		
}

/* make vector field  */
void LIC :: vectorField(int xSize,  int  ySize,  float*  vectr)
{   		
		for(int j = 0;  j < ySize;  j++)
    	for(int i = 0;  i <xSize;  i++)
		{	
			int	 index = ((ySize - 1 - j) *xSize + i  )  <<  1;
			vectr[index] = FD.getDataY(i, j, 60); 
			vectr[index + 1] = FD.getDataX(i, j, 60); 	
    	}	    	
}


/*	normalize the vector field  */
void LIC :: NormalizVectrs(int xSize,  int  ySize,  float*  vectr)
{	
		for(int	 j = 0;  j < ySize;  j++)
    	for(int	 i = 0;  i <xSize;  i++)
    	{	
			int		index = (j *xSize + i) << 1;
        	float	vcMag = float(  sqrt( double(vectr[index] * vectr[index] + vectr[index + 1] * vectr[index + 1]) )  );
...
void LIC:: MakeNoise(int xSize,  int  ySize,  unsigned char*  noise)
{		
		for(int  j = 0;   j < ySize;  j++)
    	for(int  i = 0;   i <xSize;  i++)
    	{	
        	...
        	noise[j *xSize + i] = (unsigned char) r;
    	}
}


/*		generate box filter LUTs     */
void LIC :: GenBoxFiltrLUT(int  LUTsiz,  float*  lut0,  float*  lut1)
{  		
   		for(int  i = 0;  i < LUTsiz;  i++)  
			lut0[i] = lut1[i] = i;
}

/* Write display the LIC on screen Based on the  */
void LIC :: displayLIC(int xSize,  int  ySize,  unsigned char*  outDraw)
{
	for(int  j = 0;  j < ySize;  j++)
   		for(int  i = 0;  i <xSize;  i++)
		{
			unsigned  char	unchar = outDraw[j *xSize + i];
			float colorCode = unchar/255.0;
			if(colorCode>0.5)
				colorCode +=0.25;
			else if(colorCode<0.45)
				colorCode -=0.25;
			else
				colorCode +=0.1;
			...
			glBegin(GL_POINTS);
				...
				glVertex3f(j, i, 50.0f);
			glEnd();
		}		
}


/*	Flow visualization through Line Integral Convolution. This  is implemented based on 
 *  the algorithm proposed by Brian Cabral and Leith (Casey) Leedom in the paper 
 *	"Imaging Vector Fields Using Line Integral Convolution" and LIC implemntation http://www.zhanpingliu.org/research/flowvis/LIC/LIC.htm
 */
void LIC :: FlowImagingLIC(int    xSize,  int     ySize,  float*  vectr,  unsigned char*  noise,  unsigned char*  outDraw,  
		float*  lut0,  float*  lut1,  float   kernel)
{	
		...
		int		ADVCTS = int(kernel * 3);	//MAXIMUM number of advection steps per direction to break dead loops	
		...
		float*	wgtLUT = NULL;				//WeiGhT Look Up Table pointing to the target filter LUT
  		float	len2ID = (discrete_filter_size - 1) / kernel;	//map a curve LENgth TO an ID in the LUT
		...
		{	
            	...
            	while( curLen < kernel && advcts < ADVCTS ) 
	         	{
					...
					segLen = (curLen > kernel) ? ( (curLen = kernel) - prvLen ) : segLen;
					...
					texVal = noise[ int(samp_y) *xSize + int(samp_x) ];
					...
					if( clp0_x < 0.0f || clp0_x >=xSize || clp0_y < 0.0f || clp0_y >= ySize)  break;
				}	
         	}
			...
			outDraw[j *xSize + i] = (unsigned char) texVal;
		} 	
}
